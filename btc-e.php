...
switch ($ENGINE) {
	...
	case "M":
		...
		*/
		$microSellDivisor = 150; //number of Sell orders to place
		$microBuyDivisor = 150; //Number of8888888888888888888 Buy orders to place
		$microIteration = 0.15;//USD$ iteration for multiple orders
		$microAmount = 0.01; //Amount of BTC per mini buy/sell round
		$microProfit = 0.0015; //The profit threshold, this is the minimum profit 
		$microStopLoss = 0.5; //The stop loss percent, Sell out at a loss
		$microReBuy = 0.01; //The price change to rebuy after a loss
		$microBuyAmountMultiplier = engineMultiplier('double',2,$microBuyDivisor);
		
		if ($GOX){
		$microSellDivisor = 50; //number of Sell orders to place
		$microBuyDivisor = 50; //Number of Buy orders to place
		$microIteration = 0.25;//USD$ iteration for multiple orders
		$microAmount = 0.01; //Amount of BTC per mini buy/sell round
		$microProfit = 0.01; //The profit threshold, this is the minimum profit per trade
		$microStopLoss = 1.5; //The stop loss percent, Sell out at a loss
		$microReBuy = 4.8; //The price change to rebuy after a loss
		$microBuyAmountMultiplier = engineMultiplier('rand',1,$microBuyDivisor);
		}
		...
		$microThreshold = 0.60; // dynamic threshold defined arbitrarily.
		$microTicker = 1; //This stores the last price used to calculate mini-trades
		break;
	case "B":	
		$BALANCE = TRUE;
...
$maxUSD = 1000; //The maximum amount of USD to use -> includes B held * last price!
...
$tradeIteration = 0.10; //If not using compound interest, the amount to differ between trades when using sell/buy divisor above 1
$compoundInterest = FALSE; //Use $incriment or profit^(1+x)
$threshold = 0.60; //The price change threshold UNUSED as this is set later.
$profitTrade = 0.021; //The profit threshold, this is the minimum profit per trade to make,
...
$emailRCPTo = "";
$btceKey = 'lolololo-0FTWPTKO-T91GP1Q8'; // your API-key
$btceSecret = 'lololol0228932164895a2bc07f3ff7d'; // your Secret-key

$emailSubject = "BTCbot Trade on {$exchange}";
 
//Time between runs 
$nanosleep = 50000000; //nanoseconds
$sleepCancel = 50000; //nanoseconds to sleep between order cancellations
////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////
	if ( $ticker['ticker']['last'] > 1 ){ //if the ticker is valid
		...
		$microThreshold = ( $ticker['ticker']['last']*( (1/(1-$exchangeCommision-($microProfit/2)) -1))) ;
...
//DEBUG gox compatibility
		...
		if (@$costBalance['shares'] > 0){
			...
			$stop['target'] = $costBalance['ideal']*.996*(1-($microStopLoss/100));
		} else $stop['target'] = 0;
...
//DEBUG check funds for buy and sell trades.
			if ( $BTCB >= $minBidBTC ){
				...
				while ( $tmp1 <= ($BTCB/$engineAmount)*$microSellDivisor && $microBTCRemaining >= $minBidBTC ){
					...
					$rate[$countOrder] = $microSellPrice + 0.1*$tmp1;
	...
	if (@$executeTrade ){
		...
		while ( $countTrades <= $countOrder ){
			if ( !empty($type[$countTrades]) ){
				...
				// The message
				$message = $message . "Type:{$type[$countTrades]} Amount:{$amount[$countTrades]}BTC at Rate:{$rate[$countTrades]} {$costBalance['ideal']}\n";
				if (@$GOX) sleep(rand(1,10));
			}
			$countTrades++;
		}
		$message = $message . "\n BTC Balance:{$BTCB} USD Bal:{$USDB} {$BTCB} x {$amount}\n";
...
////////////////////////////////////////////////////////////////////
	...
	function mtgox_query($path, array $req = array()) {
	 $key = '364dedf8-be14-4bed-aa09-772461d8b7cc'; //Your API Key
        $secret = 'vBgOd5OfsJbjOuYbATfMV4I1gJmT1njj2aWhZZ5MVHz1h13QQKL41NfGLFLuc2b1ueLU1qooC5DDfsubo2rXsA=='; //Your API Secret
